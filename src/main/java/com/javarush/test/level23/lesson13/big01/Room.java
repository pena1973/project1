...
//        г) проверить, если змея встретила мышь - то съесть ее.
        ...
        // получаю координаты головы
        int newX = snake.getSections().get(0).getY();
        int newY = snake.getSections().get(0).getX();

        // проверяем если уперлась
        if (snake.getDirection().equals(SnakeDirection.DOWN))
        newX++;
        if (snake.getDirection().equals(SnakeDirection.UP))
        newX--;
        if (snake.getDirection().equals(SnakeDirection.RIGHT))
        newY++;
        if (snake.getDirection().equals(SnakeDirection.RIGHT))
        newY--;

      //  snake.getAlive(checkSnake(newX, newY));

        if (!snake.isAlive()) return;

        // если проверку на жизнь-смерть прошли
       // а) добавить кусочек змеи в начало списка sections?
        snake.getSections().add(0, new SnakeSection(newX, newY));
        // отрезали хвост
        snake.getSections().remove(snake.getSections().size() - 1);

        // сьели мышь если она была
        if ((mouse.getX()==newX)&(mouse.getY()==newY)) eatMouse();

    }

    // если все хорошо то истина если вылезла за границы ложь
    private boolean checkSnake(int x, int y)
    {
        //если уперлась в стены
        if (width < x) return false;
        if (0>y) return false;
        if (width < x) return false;
        if (0>y) return false;

        //если уперлась в себя
        for (int i = 0; i <snake.getSections().size() ; i++)
        {
            if ((x==snake.getSections().get(i).getX())
             &  (y==snake.getSections().get(i).getY())) return false;
        }
